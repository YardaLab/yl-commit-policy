...
describe("validator", () => {
  ...
  it("accepts scope with hyphen", () => {
    const result = validateCommit("feat(auth-api): YLC-42 add token validation");
  ...
  it("fails for unsupported commit type", () => {
    const result = validateCommit("homelander(core): YLDTE-1 test");
  ...
  it("fails for uppercase commit type", () => {
    const result = validateCommit("FEAT(core): YLDTE-1 test");
  ...
  it("returns INVALID_FORMAT for malformed commit with unsupported type", () => {
    const result = validateCommit("homelander(): YLDTE-1 ");
